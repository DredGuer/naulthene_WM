...
| **Le plafond est-il un plancher géométrique ?** | ✅ **Non — mesuré le 30/08/2026.** Sur `SimpleCrossingS9N1` un marcheur aléatoire fait **5,67 %** (600 épisodes : 4,50 %, IC95 [3,1 ; 6,5]) quand les cerveaux entraînés atteignent **25,83 %** agrégé (`z = +13,56`), et l'un d'eux **37,33 %** — dans la fourchette de PPO (27–40 %). **La compétence est réelle.** ⚠️ Mais les victoires restent **browniennes** : 14,2× à 18,1× le plus court chemin (trajet optimal médian **12 pas**, budget 324 ticks), et `r(succès, directivité) = −0,92`. L'agent gagne bien plus souvent que le hasard sans jamais parcourir une trajectoire dirigée. 🔴 **Étendu à n = 20 le 31/08/2026, et il a livré le premier prédicteur significatif du dépôt** : la **directivité** — longueur du trajet rapportée au plus court chemin réel — prédit le succès à **`r = −0,8225`** (`t = −5,96`, n=19), soit **68 % de la variance**, contre 16 % pour la maîtrise en run. Trois vérifications passées : pas de saturation de budget (plafond 27,0×, pire cerveau 22,83×), pas de tautologie (`B_g122` fait 0,00 % de succès sans **aucune** directivité définie), et le lien survit au retrait des quatre extrêmes (`r = −0,789`, `t = −4,63`, n=15 — ⚠️ valeurs **corrigées le 02/09**, la version publiée le 31/08 annonçait `−0,78`/`t = −3,27` sur une cohorte partielle). **Ce qui sépare un cerveau à 3 % d'un cerveau à 37 % est son coefficient de diffusion spatiale** — ni la perception, ni la taille, ni le métabolisme, ni le barème. ⚠️ **Une rétractation** : une version antérieure de cette ligne rapportait une *inversion* (`r = −0,89`) entre maîtrise et score au banc — c'était un **biais de sélection de ma part**, quatre cerveaux tous tirés du haut de la distribution. À n=20 la corrélation vaut **+0,3961** (NS). La maîtrise n'est pas inversée, elle est **bruitée** : elle explique 16 % de la variance, et à maîtrise égale deux cerveaux vont de 3,00 % à 28,67 %. 🔴 **REJEU SUR LE BANC CORRIGÉ (02/09/2026, 20/20) — et le chiffre est REQUALIFIÉ.** Ces chiffres provenaient d'une sonde qui lisait la mémoire de travail au mauvais index ; l'agent jouait **sans mémoire de travail ni contexte épisodique**. Re-mesurée sur toute la cohorte, la directivité **survit mais s'affaiblit** : `r = −0,6794` (`t = −3,93`, n=20), **46 % de la variance** — et elle **ne survit plus au retrait des quatre extrêmes** (`r = −0,478`, `t = −2,04`, **NS**), qui était l'une des trois vérifications ayant établi le résultat le 31/08. La corrélation est **portée par ses extrêmes** : elle sépare les cerveaux très directifs des très browniens, elle discrimine mal au milieu. ⚠️ Ce point a été consigné **à n=15, avant la fin de la campagne**, pas découvert après coup. Vérifications qui passent : témoin aléatoire **5,67 % sur 20/20**, pas de saturation (pire 26,25× contre un plafond de 27,0×), aucun cerveau à zéro victoire. Et **la mémoire de travail est une source de variance, pas un levier** : δ succès **+0,63 pt** (`t = +0,40`, 10/20) en moyenne, mais **A_g111 +17,0 pt**, **B_g11 +17,3**, **A_g166 −10,7** individuellement — tout classement de cerveaux fondé sur les chiffres des 30-31/08 est **caduc** ([rejeu](docs/recherche/campagnes/REJEU_02092026_la_directivite_survit_affaiblie.md)) |
...
| **Pourquoi ça plafonne au niveau 4** | 🔴 **INCONNU — et la réponse métabolique a été retirée le 29/08/2026.** `maîtrise ~ énergie moyenne` a été citée neuf jours durant comme **r = +0,710** (`t = +2,85`) — mesurée à **n = 10**. À **n = 20** elle vaut **r = −0,0588 (`t = −0,25`)** : le signe s'inverse, le signal disparaît, et un jackknife montre qu'aucune graine ne le portait (r reste dans [−0,168 ; +0,055]). g77 a la 2ᵉ meilleure énergie et 17,3 % de maîtrise ; g144 la 2ᵉ pire et 22,5 %. **Dix-huit explications mesurées, dix-huit réfutées.** Trois sont tombées le 30/08/2026 : la **récompense creuse** (la prémisse est fausse — **86 % du signal est dense**, et normaliser par épisode mesure **pire**, 60 tirages sur 60), la **curiosité** (rente permanente confirmée à 40 % du signal, mais maîtrise **15,0 % contre 15,0 %** entre curiosité faible et forte), et le **barème** lui-même, qui s'est révélé une tautologie. Ce qui reste vrai et inexpliqué : trois **constantes posées** calibrent encore le rythme métabolique d'un agent **nouveau-né** (4 épisodes/jour supposés, 1,55 joués), et **9 graines sur 10 sont au plafond exact de `PATIENCE_MAX = 350`** |
...
**Aucune cause mesurée ne survit.** Dix-huit explications ont été testées, dix-huit réfutées.
