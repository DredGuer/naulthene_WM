...
| 🔴 **v41.35 — le plafond est GÉOMÉTRIQUE** | L'œil **distingue** ressource et mur (cos = **0,610** sur l'observation brute) ; le réseau les **confond** ensuite : `bus_latent` 0,959, `pensee_bio` **0,996**. Le gradient d'une couche linéaire valant `δ ⊗ x`, des entrées colinéaires donnent des gradients colinéaires — `cos(∇_res, ∇_mur) = +0,986` sur 3 cerveaux, **quelle que soit la récompense**. La borne `\|logit_r − logit_m\| ≤ ‖W‖·‖x_r − x_m‖` plafonne l'action favorisée à **14,56 %–18,00 %** contre **14,29 %** pour le hasard. Les 15,00 % mesurés en jeu ne sont **pas de l'apathie — c'est le maximum que la géométrie autorise.** Les quatorze pistes réfutées agissaient toutes sur `δ` ; aucune ne pouvait agir sur `x` |
