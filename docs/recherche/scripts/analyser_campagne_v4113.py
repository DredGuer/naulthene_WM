...
sous-échantillonné, et affiche systématiquement le recouvrement des intervalles.
    ...
    PYTHONPATH=src python docs/recherche/scripts/analyser_campagne_v4113.py [/tmp/campagne_v4113]
"""
...
def main() -> int:
    racine = Path(sys.argv[1] if len(sys.argv) > 1 else "/tmp/campagne_v4113")
    logs = racine / "logs"
    if not logs.is_dir():
    ...
    print("=" * 78)
    print("CAMPAGNE v41.13 — le corps dans le rollout de C2 sert-il à quelque chose ?")
    print("  témoin   = --sans-corps-rollout (C2 simule SANS corps, v41.12)")
    print("  variante = par défaut           (C2 reprojette le vecteur bio, v41.13)")
    print("=" * 78)
