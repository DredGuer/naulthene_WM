...
| | |
|---|---|
| **Début** | **2026-09-15 09:46:49** — dossier de campagne créé à 09:46:49 (`date`), `LISEZ_MOI.md` et `empreintes_avant.txt` écrits, **AVANT le premier processus**. Entrée ouverte à 09:47, avant le lancement |
| **Fin estimée** | **~11:05** (≈ 76 min) — dérivée du rythme **MESURÉ** (`sonde_cadence.txt` du premier pilote) : 0,63 s/épisode sur la carte 3 et 0,32 sur la carte 4 ⇒ `4 cerveaux × 1 145 épisodes × 0,95 s` ≈ **4 351 s ≈ 73 min**, plus 8 chargements de `.brain` à 0,43 s |
| **Fin réelle** | **2026-09-15 11:03:41** — run de **4 536,6 s (75 min 36 s)** (09:48:01 → 11:03:41), soit **≈ 2 min d'avance** sur l'estimation (~76 min) : la cadence du premier pilote s'est vérifiée |
| **Coût** | 4 cerveaux `K8_NU` (graines 11, 22, 33, 44 — **les mêmes** que les pilotes antérieurs) × 2 cartes figées × **1 145 épisodes** (au lieu de 200), graines d'éval **10000…11144**, `NAULTHENE_DEVICE=cpu`, cohorte explicite (choix, pas obligation), lecture seule |
| **Pourquoi** | « À 200 épisodes par carte, l'IC95 de σ̂ touche 0 : la dispersion inter-cerveaux est-elle identifiable si l'on porte le budget à ~1 145 épisodes par carte (condition `σ²/v > χ²/df − 1 = 2,1161`), et quel `n_final = max(n_carte3, n_carte4)` cette dispersion impose-t-elle ? » |
| **Résultat** | **Condition de validité TENUE : l'IC95 de σ̂ exclut zéro sur les deux cartes.** Carte 3 : σ̂ = **0,034303**, IC95 **[0,015257 ; 0,138238]**, `n` = **1 868**. Carte 4 : σ̂ = **0,077477**, IC95 **[0,043056 ; 0,291250]**, `n` = **184**. **`n_final = max(1868, 184) = 1 868`** (la carte 3 contraint). ρ inter-cartes = **−0,8958** (confirme le −0,88) ⇒ cartes séparées, jamais poolées. Coût tâche 9 : **149 440 épisodes ≈ 20,8 h** (2 bras × 20 × 2 cartes). Carnet : [`docs/recherche/campagnes/EVA01_N1145_15092026_la_dispersion_identifiee.md`](../../docs/recherche/campagnes/EVA01_N1145_15092026_la_dispersion_identifiee.md) |
