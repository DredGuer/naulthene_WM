...
| **[CHANTIER_VIS-01_cerveau_3d_irm_vivante](ameliorations/CHANTIER_VIS-01_cerveau_3d_irm_vivante.md)** | 🟡 **VIS-01 — LE CERVEAU 3D : une IRM vivante.** Spec **validée le 10/09/2026**, aucune ligne écrite. Le cerveau n'est observable qu'à plat (courbes, barres, heatmap 2D) : VIS-01 rend visibles ses **12 couches** et ses **220 255 synapses** (mesuré, `dim_bus` = 145), allumées par l'activité réelle — puis branchées sur **un run en cours par UDP** (étape 2, drapeau additif dans `noyau.py`, éteint par défaut). Rendu **three.js vendorisé** (aucune dépendance ajoutée), rapporteur par **forward hooks** (discipline MES-02). 🔴 **Contient deux découvertes hors périmètre** : la garantie « ne modifie aucun poids » de `irm_cerveau.py`/`lancer_arene.py` est **inexacte** (`fortifier_synapses` écrit sous `no_grad`, sans `backward` — mesuré nul seulement parce que la trace est nulle), et l'en-tête `#Version actuelle 41.68` diverge du CHANGELOG (**v41.75**) |
| **[PLAN_VIS-01_cerveau_3d](ameliorations/PLAN_VIS-01_cerveau_3d.md)** | 📐 **Le plan d'implémentation de VIS-01** — 10 tâches atomiques en TDD, chacune avec ses tests, ses commandes et son résultat attendu : codec de trames → bus borné → serveur stdlib → page three.js → source factice (étape 0) → rapporteur → spectateur-pilote (étape 1) → drapeau `--telemetrie-3d` dans `noyau.py` (étape 2) → documentation. ⚠️ La tâche 9 est **bloquée par la décision D2** (numéro de version du CHANGELOG) |
