...
| 🔴 **v41.36 — the head chases a moving target** | Everything works: the trunk **amplifies** the decisive feature (d' 0.824 → 2.891/3.613/3.017), the gradient arrives (0.234/0.056), **g22 is never clipped** (0/5 nights), Adam takes its full step (0.93 % of \|W\| per 5 nights), the night does not erode (norm at **90–98 %** of birth), and the steps are **directed** (\|cos(ΔW, axis)\| up to **9.1× chance**) and **additive** (temporal alignment **0.969** vs 0.316 for a random walk). But the informative axis — defined in `pensee_bio`, trained *at the same time* — **rotates 4.81°/night** while `W` closes at 0.106°/night: the target flees **14× to 46× faster** than the pursuer. **Representation drift**, never measured here before |
