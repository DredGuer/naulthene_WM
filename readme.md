...
| Effect of severing C2 on the score | **0.0 points across all 6 levels** (78 cells) — and on `LavaGap`, severing it **triples** the success rate. 🔴 **CONFOUNDED RESULT (5 Sep 2026)**: the `c2_coupe` lesion sets `force_planification = 0`, which pins `gain_c1` to its lower bound **0.25** — it severed C2 **and throttled C1 to a quarter**. It also exists **only in the ablation bench**, never in the curriculum. A clean ablation (`--sans-c2`, v41.57) is **under way** |
