...
| 🔴 **v41.35 — the ceiling is GEOMETRIC** | The eye **distinguishes** resource from wall (cos = **0.610** on the raw observation); the network then **confuses them**: `bus_latent` 0.959, `pensee_bio` **0.996**. Since a linear layer's gradient is `δ ⊗ x`, colinear inputs give colinear gradients — `cos(∇_res, ∇_wall) = +0.986` on 3 brains, **whatever the reward**. Bound: `\|logit_r − logit_m\| ≤ ‖W‖·‖x_r − x_w‖` caps the favoured action at **14.56 %–18.00 %** against **14.29 %** for chance. The 15.00 % measured in play is **not apathy — it is the maximum geometry allows.** Every one of the fourteen refuted leads acted on `δ`; none could act on `x` |
